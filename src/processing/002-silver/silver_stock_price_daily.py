...
bronze_df = bronze_df.selectExpr("symbol as Symbol", "cast(Timestamp as date) as TradeDate", "CAST(open as DOUBLE) as Open", "CAST(high as DOUBLE) as High", "CAST(low as DOUBLE) as Low", "CAST(close as DOUBLE) as Close", "CAST(volume as BIGINT) as Volume", " cast(date_format(Timestamp, 'yyyyMM') as INT) as TradeYearMonth")
#display(bronze_df  )
