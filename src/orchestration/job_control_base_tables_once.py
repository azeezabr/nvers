...
sql = f"""
    select * from delta.`{backfill_control_table_path}`;
"""
...
def check_file_exists(year, month, day):
    ...
    directory_path = f"bronze/overview/year={year}/month={str(month).zfill(1)}/day={str(day).zfill(1)}"
        ...
        if check_file_exists(year, month, day):
            ...
            if job_id:
                ...
                if run_id:
                    ...
                    log_job_details(job_id, run_id, current_date.date(), start_time, end_time, status, existing_cluster_id, "/Path/To/Notebook", "")
